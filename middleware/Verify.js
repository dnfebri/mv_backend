...
export const verifyToken = (req, res, next) => {
  if(!req.header('authorization')) return res.status(401).json(
    responseJson(
      false, "'Authorization Bearer' Token not detected"
    )
  );
  const token = req.header('authorization').split(" ")[1];
  if(!token) return res.status(401).json(responseJson(
    false, "Token not detected"
  ));
  jwt.verify(token, process.env.TOKEN_SECRET, (err, decoded) => {
    if(err) return res.status(403).json(responseJson(
      false, err.message
    ));
