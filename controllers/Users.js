...
export const updateUser = async (req, res) => {
  ...
  if (user.email !== email) {
    ...
    if (emailUser)
      return res.status(404).json(responseJson(false, "Email already exists"));
  }
  ...
  try {
    ...
    res.status(200).json(responseJson(true, "Successfully Update User", user));
  } catch (error) {
...
export const changePassword = async (req, res) => {
  ...
  if (newPassword !== confrimNewPassword)
    return res
      .status(400)
      .json(responseJson(false, "Password confirmation does not match!"));
  ...
  try {
    ...
    res.status(200).json(responseJson(true, "Successfully Change Password"));
  } catch (error) {
    res.status(500).json(responseJson(false, error));
  }
};
